...
class PandaImageAndLabelDataset(Dataset):
    ...
    train_root = 'image_train'
    test_root = 'image_test'

    train_annotation_file_path = 'image_annos/person_bbox_train.json'
    test_annotation_file_path = 'image_annos/person_bbox_test_real.json'

    def __init__(self, cfg, panda_root='', split='train', target_key=VISIBLE_BODY_KEY):

        self.cfg = cfg
        self.__initialize_category()

        image_root = os.path.join(panda_root, PandaImageAndLabelDataset.train_root)
        annotation_file_path = os.path.join(panda_root, PandaImageAndLabelDataset.train_annotation_file_path)
        if split != 'train':
            image_root = os.path.join(panda_root, PandaImageAndLabelDataset.test_root)
    ...
    def __getitem__(self, index):
        ...
        t0 = time.time()
        if self.cfg.DATASETS.PANDA.REDUCE_INPUT:
            img_origin = cv2.imread(image_path, flags=cv2.IMREAD_REDUCED_COLOR_8)
        else:
            img_origin = cv2.imread(image_path)
        DEFAULT_LOGGER.debug(f"[Dataset] after cv2.imread cost {time.time() - t0}s")
        ...
        DEFAULT_LOGGER.debug(
            f'image_path: {image_path}, original_height: {original_height}, original_width: {original_width}')
        ...
        DEFAULT_LOGGER.debug(f"[Dataset] after cv2.resize cost {time.time() - t0}s")
        ...
        DEFAULT_LOGGER.debug(f"[Dataset] after from_numpy cost {time.time() - t0}s")
        ...
        if number_labels:
            labels_out[:, 1:] = labels
        DEFAULT_LOGGER.debug(f"[Dataset] after process label {time.time() - t0}s")
    ...
    def __collect_labels(self, annotation_file_path, target_key=VISIBLE_BODY_KEY):
        ...
        for panda_image in self.image_list:
            ...
            panda_image.label_list = convert_to_label_ltrb_list(panda_image.objects_list, self.category_name_index_dict,
    ...
    def collate_fn(batch):
        ...
        img0, labels0, img_origin0 = torch.stack(img, 0), torch.cat(labels, 0), torch.stack(img_origin, 0)
        return img0, labels0, img_origin0
